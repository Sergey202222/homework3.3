...
{
    for (int iindex{1}; iindex < size; ++iindex)
    {
        for (int jindex{iindex}; jindex > 0; --jindex)
        {
            if (addresses[jindex].get_output_address()[0] < addresses[jindex - 1].get_output_address()[0])
            {
                std::swap(addresses[jindex], addresses[jindex - 1]);
            }
        }
    }
}
...
int main()
{
    ...
    sort(addresses, numberOfAddresses);
